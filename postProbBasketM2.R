postProbBasketM2<-function(PI,pM,n1,n2,alphaFut=0.20, cutEff=0.650,
...
{ 
  ...
  # alphaFut=0.20;alphaEff=0.30;q0=0.20;q1=0.35;n1=20;n2=20;aP=1;bP=1;J=3;K=4
  qFut=qbinom(1-alphaFut,n1,q0) # when q0 ==>[1] 0.04572541(under alphaFut)but(under 0.4614986 under 1-alphaFut), when q1 ==>[1] 0.4614986    # qFut is non-effective at exact cutFut, under n1 data assumtion alphaEff is predetermined
  ...
  for (iSimu in 1:numSimu) {       # main loop
    ...
    for (j in 1:J) for (k in 1:K) 
    {postProb2[j,k]=pbeta(q1,aP+R[j,k],bP+N[j,k]-R[j,k],lower.tail = FALSE)}   #PRIOR alpha=beta=12 instead of 1
    ...
    rowEffFLAG=(rowPostProb>row_cutEff)*1     # flag vector of row efficacy after stage 2
    colEffFLAG=(colPostProb>col_cutEff)*1     # flag vector of column efficacy after stage 2
    
    # final product of pi-jk with indicator functions
    for (j in 1:J){
      for (k in 1:K){ 
        effFLAG_enhanced[j,k]=effFLAG[j,k]*max(rowEffFLAG[j],colEffFLAG[k]) 
        Q[j,k]=PI[j,k]*effFLAG_enhanced[j,k]     # for PCSk 
      }
    }
    ...
    if(max(Q)!=0){
      if( length(unique(rowSums(Q)))!=1){
        if( length(unique(rowSums(Q)))==3){jStarHat[iSimu]=which.max(rowSums(Q))}else{jStarHat[iSimu]=3} #if( (length(unique(rowSums(Q)))==2)&(which.max(rowSums(Q))==2) ){jStarHat[iSimu]=3}
    ...
    for(k in 1:K) if(max(Q[,k])!=0){jStarHat_k[iSimu,k]=which.max(rowSums(PI)*effFLAG_enhanced[,k])}else{jStarHat_k[iSimu,k]=NA}  # YYYYY rowSums(PI)*effFLAG_enhanced # for PCSk ==> Prev. Q[,k]
